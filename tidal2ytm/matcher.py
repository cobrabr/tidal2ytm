...
from .models import SourceTrack, MatchResult
    ...
    for candidate in candidates:
        ...
        if track.isrc:
            try:
                ...
                c_isrc = _isrc_from_song_detail(detail)
        ...
        # Strategy 2+3: Duration + fuzzy album
        if c_dur is None:
            continue
        if abs(c_dur - track.duration_sec) > DURATION_TOLERANCE_SEC:
            continue
        ...
        if conf > best_confidence:
            ...
            best_video_id = vid
            best_meta = {"title": c_title, "artist": c_artist, "album": c_album, "dur": c_dur}
            best_method = "duration" if album_sim < 0.3 else "fuzzy"
