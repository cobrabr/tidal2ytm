...
1. `tidal2ytm` (no args) — interactive planning TUI (`planning.py`): fetch `tidal_source.py:get_liked_tracks` once → search/filter locally → accumulate a selection → explicit match via `matcher.py:match_track` → merge into `data/transfer_plan.toml`. Done when the selection is matched, `transferred` tracks are skipped, and `[meta]` is recomputed via `plan_io.py:update_plan_meta`. Improved matches prompt `[y/N]` per track unless the `ctrl+o` override banner is on.
...
- `[meta]` is recomputed via `plan_io.py:update_plan_meta()` after any status change. `transfer.py`, `review.py`, and `planning.py` are responsible for calling it through `save_plan`.
