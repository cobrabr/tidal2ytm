...
def test_review_confidence_color_and_navigation(tmp_path: Path):
    ...
    assert review_mod._confidence_color(0.75) == "yellow"
...
def test_review_confidence_color_boundaries():
    ...
    assert review_mod._confidence_color(0.70) == "yellow"
    assert review_mod._confidence_color(0.69) == "red"
...
def test_build_track_context_and_navigation(isolated_data_dir: Path):
    ...
    assert review_mod._next_artist_cursor(session) == 3

    # prev album from index 2 should go to 0
    session.cursor = 2
    assert review_mod._prev_album_cursor(session) == 0
    ...
    session.cursor = 3
    assert review_mod._next_album_cursor(session) == 3
    assert review_mod._next_artist_cursor(session) == 3
    session.cursor = 0
    assert review_mod._prev_album_cursor(session) == 0
    assert review_mod._prev_artist_cursor(session) == 0
...
def test_review_backup_on_first_write(isolated_data_dir: Path):
    ...
    with patch("tidal2ytm.review.backup_plan", wraps=plan_io.backup_plan) as mock_backup:
        review_mod._apply_decision(session, track, TrackStatus.SKIP.value)
        ...
        review_mod._apply_decision(session, track, TrackStatus.PENDING.value)
...
def test_review_do_override_parses_url(isolated_data_dir: Path, monkeypatch):
    ...
    console = Console()
    review_mod._do_override(console, session, track)
