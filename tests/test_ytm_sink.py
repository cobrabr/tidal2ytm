...
def test_ytm_sink_dry_run_does_not_call_api(monkeypatch):
...
def test_ytm_sink_no_video_id_returns_false(monkeypatch):
    monkeypatch.setattr("tidal2ytm.ytm_sink.time.sleep", lambda _: None)
    yt = MagicMock()
    assert add_track_to_library(yt, "", "Bungle", dry_run=False) is False


def test_ytm_sink_no_add_token_returns_false(monkeypatch):
    monkeypatch.setattr("tidal2ytm.ytm_sink.time.sleep", lambda _: None)
    yt = MagicMock()
...
def test_ytm_sink_success_calls_edit(monkeypatch):
    monkeypatch.setattr("tidal2ytm.ytm_sink.time.sleep", lambda _: None)
    yt = MagicMock()
...
def test_ytm_sink_exception_returns_false(monkeypatch):
    monkeypatch.setattr("tidal2ytm.ytm_sink.time.sleep", lambda _: None)
    yt = MagicMock()
...
def test_ytm_sink_no_tracks_returns_false(monkeypatch):
    monkeypatch.setattr("tidal2ytm.ytm_sink.time.sleep", lambda _: None)
    yt = MagicMock()
